...
export class ImagonyScraper {
  private agentId: string;
  private apiKey: string;

  constructor(agentId: string = 'AGENT_1770234357951_52D732', apiKey?: string) {
    this.agentId = agentId;
    this.apiKey = apiKey || process.env.IMAGONY_API_KEY || '';
  }

  async scrapeAgentData(): Promise<ImagonyAgentData | null> {
    ...
    try {
      ...
      if (this.apiKey) {
        ...
        await page.evaluate((key: string) => {
          (window as any).localStorage.setItem('agent_token', key);
          (window as any).localStorage.setItem('agent_id', 'AGENT_1770234357951_52D732');
        }, this.apiKey);
  ...
  async fetchFromApi(): Promise<ImagonyAgentData | null> {
    try {
