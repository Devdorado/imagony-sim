...
- `imagony.com` liegt im selben Cloudflare-Konto. Der Apex-Record zeigt als proxied CNAME auf `imagony.pages.dev`; `www` ebenso. Alle vier Pages Custom Domains sind im Dashboard aktiv und SSL-fähig. Eine Cloudflare Bulk Redirect Rule leitet `www` mit 301, Pfad und Query auf den Apex um. `market.imagony.com` und `chat.imagony.com` wurden als Pages Custom Domains hinzugefügt; Cloudflare setzte für beide proxied CNAMEs auf `imagony.pages.dev`. `chat` zeigte zuvor als proxied A auf `76.13.142.152` und lieferte HTTP 522. Eine aktive zoneweite Single Redirect Rule mit der Expression `http.host in {"chat.imagony.com" "market.imagony.com"}` leitet beide Hosts mit 301 auf die kanonische Seite `https://imagony.com/marketplace/`; Query-Parameter werden verworfen. Cloudflares öffentlicher DNS-Resolver lieferte den neuen `market`-Record bereits, während der lokale Speedport-Router noch die zuvor abgefragte NXDOMAIN-Antwort zwischenspeicherte.
...
Das Produktionsdeployment und die Custom Domains wurden per HTTP geprüft: statische Seiten, Assets, API, D1, Registrierung, Moderation, Löschung, alte URL-Weiterleitungen, `www`-Weiterleitung und 404-Verhalten. Der Marketplace wurde zusätzlich lokal und auf der GitHub-Branch-Preview über D1, Moderation und Kontakt-Privatsphäre geprüft; synthetische Preview-Daten sind gelöscht. Nach dem Merge von PR #5 am 24.09.2026 wurden `https://imagony.com/marketplace/` und `/api/listings` mit HTTP 200 sowie beide Alias-Hosts über Cloudflare-Edge-IP mit HTTP 301 geprüft. Nach einem neuen Push dieselben Kernwege bei Bedarf erneut prüfen. [Cloudflare Custom Domains](https://developers.cloudflare.com/pages/configuration/custom-domains/).
...
Als erster echter Eintrag wurde Marc Lanz / Scintil mit der ID `950c2ab5-3e73-4319-9580-fa07cdc62f06` nach redaktioneller Freigabe veröffentlicht. Sein einmaliger Management-Token liegt ausschließlich in der ignorierten lokalen Datei `docs/imagony-marketplace-owner-2026-09-24.md` mit Modus 0600. Vor dem Entfernen dieses Checkouts sicher in einen Passwortmanager übernehmen; den Token nicht in Git, Tickets oder öffentlichen Chat kopieren.
